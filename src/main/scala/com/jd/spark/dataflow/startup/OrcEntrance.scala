...
import com.jd.spark.dataflow.kafka2orc.consumers.SparkConsumerKafka
...
object OrcEntrance {
  ...
  def main(args: Array[String]): Unit = {
    ...
    opt.setRequired(true);
    opts.addOption(opt);
    opt = new Option(MyConstants.RULES_FILE_PATH, MyConstants.RULES_FILE_PATH, true, "规则文件路径");
    opt.setRequired(true);
    opts.addOption(opt);
    ...
    try {
      var argsTest = Array("-service", "kafka2hdfs", "-consumer_topic", "serverinfo", "-kafka_broker_id", "1", "-rules_file_path", "/spark/1.txt")
      ...
      if (cl.getOptions().length > 0) {
        if (cl.hasOption(MyConstants.HELP)) {
          ...
          if ("kafka2hdfs".equals(cl.getOptionValue(MyConstants.SERVICE))) {
            ...
            //验证path是否为空
            if (cl.getOptionValue(MyConstants.RULES_FILE_PATH).isEmpty()) {
              logger.info("input param error")
              System.exit(-1);
            }
            //运行程序
