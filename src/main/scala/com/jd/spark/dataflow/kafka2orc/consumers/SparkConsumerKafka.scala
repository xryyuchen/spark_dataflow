package com.jd.spark.dataflow.kafka2orc.consumers
...
import com.jd.spark.dataflow.kafka2orc.utils.{HDFSFileUtils, LocalFileUtils, MyOffsetCommitCallback}
...
class SparkConsumerKafka(cl: CommandLine) extends Serializable {
  ...
  def  run(): Unit ={
    ...
    val rulefile_path = cl.getOptionValue(MyConstants.RULES_FILE_PATH);
    ...
    val hdfsFileUtils: HDFSFileUtils = new HDFSFileUtils();
    val beanArr: ArrayBuffer[ParseRules] = hdfsFileUtils.readFileOnHDFS(rulefile_path);
    ...
    realTimeDStream.foreachRDD { rdd =>
      ...
      if (!rdd.isEmpty()) {
        ...
        var rows:RDD[ConsumerRecord[String,String]]=null;
        var ts:Long=0;
        ...
        while(a>0){
          try{
            ...
            partition=hdfsFileUtils.getPartitionPath(topic,ts);
            ...
            ts=System.currentTimeMillis();
            if(spark_coalesce_num>0){
              rows=rdd.coalesce(spark_coalesce_num,false);
            }else{
              rows=rdd;
            }
            hdfsFileUtils.write(rows, tmpPath + MyConstants.DIR_SEPARATOR , beanArr);
            hdfsFileUtils.renameMV(tmpPath, realPath, ts,true);
            a= -1;
          }catch {
            case t: Exception => {
