...
import com.jd.spark.dataflow.utils.KafkaCluster.LeaderOffset
...
class MyKafkaManager(val kafkaParams: Map[String, String]) extends Serializable {
  ...
  private val kc = new MyKafkaCluster(kafkaParams);
 ...
 /* def createDirectStream[K: ClassTag, V: ClassTag](
    ssc: StreamingContext, kafkaParams: Map[String, String], topics: Set[String]): InputDStream[ConsumerRecord[K, V]] = {
    val groupId = kafkaParams.get("group.id").toString()
    ...
    val messages = {
      ...
      consumerOffsets.foreach(m=>{
         topicPartition+= (new TopicPartition(m._1.topic, m._1.partition)->m._2);
      });
  ...
  def getTopicPartition[K: ClassTag, V: ClassTag](
    ssc: StreamingContext, kafkaParams: Map[String, String], topics: Set[String]): Map[TopicPartition,Long] = {
