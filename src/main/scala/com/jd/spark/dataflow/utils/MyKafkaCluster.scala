...
class MyKafkaCluster(val kafkaParams: Map[String, String]) extends Serializable {
  ...
  def config: SimpleConsumerConfig = this.synchronized {
    if (_config == null) {
      _config = SimpleConsumerConfig(KafkaCluster.mergeKafkaParams(kafkaParams))
    }
    _config
  }
...
object KafkaCluster {
  ...
  def mergeKafkaParams(kafkaParams:Map[String, String]):Map[String, String]={
     var oldKafkaParams:Map[String,String]=Map("a"->"b");
     kafkaParams.foreach(f=>{
       if("auto.offset.reset" == f._1.toLowerCase() && "latest" == f._2.toLowerCase()){
          oldKafkaParams+= ("auto.offset.reset"->"largest");
       }else{
          oldKafkaParams+= (f._1->f._2);
       }
     });
     return oldKafkaParams;
  }
  ...
  object SimpleConsumerConfig {
    ...
    def apply(kafkaParams: Map[String, String]): SimpleConsumerConfig = {
      val oldKafkaParams=KafkaCluster.mergeKafkaParams(kafkaParams);
      // These keys are from other pre-existing kafka configs for specifying brokers, accept either
      val brokers = oldKafkaParams.get("metadata.broker.list")
        .orElse(oldKafkaParams.get("bootstrap.servers"))
        .getOrElse(throw new SparkException(
          "Must specify metadata.broker.list or bootstrap.servers"))

      val props = new Properties()
      oldKafkaParams.foreach {
        case (key, value) =>
